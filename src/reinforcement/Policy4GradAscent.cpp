...
namespace whiteice
{
  ...
  template <typename T>
  Policy4GradAscent<T>::~Policy4GradAscent()
  {
    start_lock.lock();
					  ...
					  bool initiallyUseNN)
  {
    ...
    if(data_->size(0) < 1){
      logging.error("Policy4GradAscent:startOptimize(): data_->size(0) < 1 error");
      return false;
    }
	...
	start_lock.unlock();
	logging.error("Policy4GradAscent::startOptimize(): thread_is_running > 0 error");
	return false;
      }
    }
	...
	this->policy->calculateBatchNorm(xdata, 2);
      ...
      whiteice::logging.info("Policy4GradAscent: input Q weights diagnostics");
      this->Q->diagnosticsInfo();

      whiteice::logging.info("Policy4GradAscent: input policy weights diagnostics");
    ...
    for(unsigned int i=0;i<optimizer_thread.size();i++){
      optimizer_thread[i] =
	new std::thread(std::bind(&Policy4GradAscent<T>::optimizer_loop,
				  this));
    }
  ...
  template <typename T>
  bool Policy4GradAscent<T>::isRunning()
  {
   ...
   */
  template <typename T>
  bool Policy4GradAscent<T>::getSolution(whiteice::nnetwork<T>& policy,
					T& value,
  ...
  template <typename T>
  bool Policy4GradAscent<T>::getSolutionStatistics(T& value,
						  unsigned int& iterations) const
  {
    std::lock_guard<std::mutex> lock(solution_lock);
  ...
  template <typename T>
  bool Policy4GradAscent<T>::getSolution(whiteice::nnetwork<T>& policy) const
  {
    std::lock_guard<std::mutex> lock(solution_lock);
  ...
  template <typename T>
  bool Policy4GradAscent<T>::getDataset(whiteice::dataset<T>& data_) const
  {
    std::lock_guard<std::mutex> lock(solution_lock);
    data_ = this->data;
    return true;
  ...
  template <typename T>
  bool Policy4GradAscent<T>::stopComputation()
  {
    start_lock.lock();
  ...
  template <typename T>
  void Policy4GradAscent<T>::reset()
  {
    start_lock.lock();
...
#pragma omp critical
  ...
  template <typename T>
  void Policy4GradAscent<T>::optimizer_loop()
  {
	...
	if(first_time == false){
	  whiteice::logging.info("Policy4GradAscent: reset policy network");
	  ...
	  if(deep_pretraining){
	    ...
	    if(deep_pretrain_nnetwork_full_sigmoid(ptr, dtrain, false, 2, &running) == false)
	      whiteice::logging.error("Policy4GradAscent: deep pretraining FAILED");
	    else
	      whiteice::logging.info("Policy4GradAscent: deep pretraining completed");
	  ...
	  whiteice::logging.info("Policy4GradAscent: use previous/old policy network");
...
#pragma omp critical
	...
	do{
	  ...
	  whiteice::logging.info("Policy4GradAscent: calculates gradient");
...
#pragma omp critical
	  ...
	  
	  {
	    ...
	    snprintf(buffer, 80, "Policy4GradAscent: raw gradient norm: %e",
		     gradlen);
	  ...
	  if(policy->exportdata(weights) == false)
	    whiteice::logging.error("Policy4GradAscent: weight export failed");
...
#pragma omp parallel for schedule(static)
	  ...
	  
	  {
	    ...
	    snprintf(buffer, 128,
		     "Policy4GradAscent (%d/%d): policy updated value %f\n",
		     iterations, MAXITERS,
		     v);
	    whiteice::logging.info(buffer);
	  }
	  ...
	  //    than the earlier one
	  {
		...
		snprintf(buffer, 128,
			 "Policy4GradAscent: better policy found: %f iter %d",
			 b, iterations);
		whiteice::logging.info(buffer);
	      }
  ...
  template class Policy4GradAscent< whiteice::math::blas_real<float> >;
  template class Policy4GradAscent< whiteice::math::blas_real<double> >;
};
