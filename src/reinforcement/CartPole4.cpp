...
namespace whiteice
{
						...
						{50,50,50,50})
  {
    ...
    // starts physics thread
    {
      ...
      running = true;
      physics_thread = new std::thread(std::bind(&CartPole4<T>::physicsLoop, this));
    }
  ...
  template <typename T>
  void CartPole4<T>::reset()
  {
    // cart-pole system system state

    {
      theta = T(0.0); // upwards angle.. [keep up problem]
      
      // starts from random angle between up and down location..
      // up-swing problem..
  ...
  template <typename T>
  bool CartPole4<T>::getState(whiteice::math::vertex<T>& state)
  {
    // whiteice::logging.info("CartPole4: entering getState()");
    ...
    // whiteice::logging.info("CartPole4: exiting getState()");
  ...
  template <typename T>
  T CartPole4<T>::normalizeTheta(const T t) const 
  {
  ...
  template <typename T>
  bool CartPole4<T>::performAction(const whiteice::math::vertex<T>& action,
				   whiteice::math::vertex<T>& newstate,
				   T& reinforcement,
				   T& distance,
				   bool& endFlag)
  {
    // whiteice::logging.info("CartPole4: entering performAction()");
  ...
  template class CartPole4< math::blas_real<float> >;
  template class CartPole4< math::blas_real<double> >;
};
