/*
 * t-SNE dimension reduction algorithm. Barnes-Hut approximation added by ChatGPT prompt.
 * Mathmatical implementation notes/documentation in docs/TSNE_BH_notes.tm (texmacs)
 * Tomas Ukkonen. 2020.
 * 
 */
...
namespace whiteice
{
  ...
  template <typename T>
  TSNE_BH<T>::TSNE_BH(const bool absolute_value)
  {
    // absolute value KL divergence should give better results (see TSNE_BH_notes.tm)
    kl_absolute_value = absolute_value;
  }

  template <typename T>
  TSNE_BH<T>::TSNE_BH(const TSNE_BH<T>& tsne)
  {
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_start(const std::vector< math::vertex<T> >& samples,
				   ...
				   VisualizationInterface* const gui)
  {
    ...
    try{
      calculate_thread = new std::thread(std::bind(&TSNE_BH<T>::calculate_loop, this));
    }
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_finished() const
  {
    std::lock_guard<std::mutex> lock(calculate_mutex);
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_get_results(std::vector< math::vertex<T> >& samples,
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate(const std::vector< math::vertex<T> >& samples,
			  const unsigned int DIM,
			  std::vector< math::vertex<T> >& results,
			  const bool verbose,
			  LoggingInterface* const messages,
...
#pragma omp parallel for schedule(auto)
	...
	printf("ERROR: TSNE_BH::calculate(): pca_p() failed with input data.\n");
	return false;
      }
      else{
	if(verbose){
	  ...
	  printf("TSNE_BH: Preprocessing linear dimension reduction %d->%d (%f var -> %f var).\n",
    ...
    // Optional: sparsify P to top-K neighbors per row for near-linear iterations
    {
      // choose K ≈ 3 * perplexity
      unsigned int TOPK = 0;
      {
        float pf = 0.0f; whiteice::math::convert(pf, PERPLEXITY);
        int k = (int)(3.0f * pf);
        if (k < 3) k = 3;
        TOPK = (unsigned int)k;
      }
      // Heuristic: only sparsify for large N
      if (xsamples.size() >= 1000) {
        sparsify_pij(pij, TOPK);
      }
    }
	...
	gui->clear();
      ...
      {
        // --- Barnes–Hut accelerated iteration ---
        {
          // Compute gradient using BH repulsion + (sparse) P attraction
          std::vector< math::vertex<T> > ygrad;
          T Z = T(0);
          if (!bh_kl_gradient(pij, yvalues, ygrad, Z, T(0.5))) {
            printf("BH gradient failed (falling back to exact method).\n");
            fflush(stdout);
            // Fallback to original exact method
            std::vector< std::vector<T> > qij;
            T qsum = T(0.0f);
          ...
          // Approximate KL divergence for diagnostics: use nonzero P entries only
          prev_round_klvalue = klvalue;
          {
            // compute Z for q as returned; then for each nonzero P term estimate q_ij
            double kl_est = 0.0;
#pragma omp parallel for schedule(auto) reduction(+:kl_est)
            for (unsigned int i=0;i<pij.size();++i){
              for (unsigned int j=0;j<pij.size();++j){
                if (i==j) continue;
                T p = pij[i][j];
                if (p == T(0)) continue;
                math::vertex<T> d = yvalues[i] - yvalues[j];
                T dist2 = (d*d)[0];
                T q_unn = T(1) / (T(1) + dist2);
                T q = q_unn / Z;
                if (q > T(0) && p > T(0)) {
                  if(kl_absolute_value)
                    kl_est += (p * whiteice::math::abs(whiteice::math::log(p/q))).c[0];
                  else
                    kl_est += (p * whiteice::math::log(p/q)).c[0];
                }
              }
            }
            klvalue = T(kl_est);
          ...
          while(history.size() > HISTORYSIZE) history.pop_front();

          T convergence_statistic = T(0.0f);
          if(history.size() >= HISTORYSIZE){
            T m = T(0.0f);
            T v = T(0.0f);
            for(const auto& k : history) m += k;
            m /= history.size();
            for(const auto& k : history) v += (k - m)*(k - m);
            v /= (history.size() - 1);
            v = sqrt(real(v));
          ...
          float klvaluef = 0.0f; whiteice::math::convert(klvaluef, best_klvalue);
          float convf = 0.0f; whiteice::math::convert(convf, convergence_statistic);

          if(iter > 0){
            snprintf(buffer, BUFLEN,
                     "%d/%d: model kl divergence %f. [%f] ETA %f hour(s) (%f minute(s))\n",
                     iter, MAXITER, klvaluef, convf, eta.estimate()/3600.0, eta.estimate()/60.0);
          } else {
            snprintf(buffer, BUFLEN, "%d/%d: model kl divergence %f [%f].\n",
                     iter, MAXITER, klvaluef, convf);
          }
          if(verbose){ printf("%s", buffer); fflush(stdout); }
          if(messages != NULL){ messages->printMessage(buffer); }

          // check if solution improved
          if(klvalue <= best_klvalue){ best_klvalue = klvalue; noimprove_counter = 0; }
          else { noimprove_counter++; }

          // checks if caller provided flag became false (=> stop)
          if(running_flag) if(*running_flag == 0) iter = MAXITER+100;

          // adaptive learning rate
          if(klvalue < prev_round_klvalue){ lrate *= T(1.5f); } else { lrate /= T(1.5f); }
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_pvalue_given_sigma(const std::vector< math::vertex<T> >& x,
...
#pragma omp parallel for schedule(auto)      
  ...
  template <typename T>
  T TSNE_BH<T>::calculate_perplexity(const std::vector<T>& pj) const
  {
...
#pragma omp critical (anvcdshoirwereio)
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_pvalues(const std::vector< math::vertex<T> >& x,
				  const T perplexity,
...
#pragma omp parallel for schedule(auto)
  ...
  template <typename T>
  bool TSNE_BH<T>::calculate_qvalues(const std::vector< math::vertex<T> >& y,
				  std::vector< std::vector<T> >& qij,
				  T& qsum) const
  {
...
#pragma omp parallel for schedule(auto)
  ...
  template <typename T>
  bool TSNE_BH<T>::kl_divergence(const std::vector< std::vector<T> >& pij,
...
#pragma omp critical (mgfdjkreowiaa)
  ...
  template <typename T>
  bool TSNE_BH<T>::kl_gradient(const std::vector< std::vector<T> >& pij,
			    const std::vector< std::vector<T> >& qij,
			    const T& qsum,
...
#pragma omp for nowait schedule(auto)
  ...
  // --------- Barnes–Hut tree utilities (quadtree/octree) ---------
  template <typename T>
  int TSNE_BH<T>::bh_build_tree(const std::vector< math::vertex<T> >& y,
                             std::vector<BHNode>& nodes) const
  {
    const unsigned int N = y.size();
    const unsigned int D = y[0].size();
    if (D < 2 || D > 3) return -1; // supported: 2D or 3D

    nodes.clear();
    nodes.reserve(8 * N);

    BHNode root;
    root.bmin.assign(D, std::numeric_limits<double>::infinity());
    root.bmax.assign(D, -std::numeric_limits<double>::infinity());
    root.child.assign((D==2)?4:8, -1);
    root.com.resize(D);
    root.com.zero();
    root.mass = T(0);
    root.point_index = -1;
    ...
    for (unsigned int i = 0; i < N; ++i) {
      for (unsigned int d = 0; d < D; ++d) {
        double v; whiteice::math::convert(v, y[i][d]);
        if (T(v) < root.bmin[d]) root.bmin[d] = v;
        if (T(v) > root.bmax[d]) root.bmax[d] = v;
      }
    }
    // make square box
    double maxside = 0.0;
    for (unsigned int d = 0; d < D; ++d) {
      maxside = std::max(maxside, (double)(root.bmax[d] - root.bmin[d]).c[0]);
    }
    for (unsigned int d = 0; d < D; ++d) {
      double center = 0.5*(root.bmin[d] + root.bmax[d]).c[0];
      root.bmin[d] = center - 0.5*maxside;
      root.bmax[d] = center + 0.5*maxside;
    }
    whiteice::math::convert(root.size, maxside);
    int root_idx = (int)nodes.size();
    nodes.push_back(root);

    auto insert_point = [&](unsigned int idx) {
      int node_i = root_idx;
      while (true) {
        BHNode& node = nodes[node_i];
        const bool is_leaf = (node.point_index >= 0) || (node.mass == T(0) && std::all_of(node.child.begin(), node.child.end(), [](int c){return c<0;}));

        if (node.point_index == -1 && is_leaf && node.mass == T(0)) {
          // empty leaf: store point here
          node.point_index = (int)idx;
          node.mass = T(1);
          node.com = y[idx];
          return;
        }
        ...
        if (node.point_index >= 0) {
          ...
          auto create_child = [&](int cid)->int{
            if (node.child[cid] >= 0) return node.child[cid];
            BHNode c;
            c.bmin = node.bmin;
            c.bmax = node.bmax;
            c.child.assign((y[0].size()==2)?4:8, -1);
            c.com.resize(D); c.com.zero();
            c.mass = T(0);
            c.point_index = -1;
            // adjust bounds for this child
            for (unsigned int d=0; d<D; ++d) {
              double mid = 0.5*(node.bmin[d] + node.bmax[d]).c[0];
              bool high = ((cid >> d) & 1) != 0;
              if (!high) c.bmax[d] = mid;
              else       c.bmin[d] = mid;
            }
            double side = 0.0;
            for (unsigned int d=0; d<D; ++d) side = std::max(side, (double)(c.bmax[d] - c.bmin[d]).c[0]);
            whiteice::math::convert(c.size, side);
            int new_idx = (int)nodes.size();
            nodes.push_back(c);
            node.child[cid] = new_idx;
            return new_idx;
          };

          auto child_id_for = [&](const math::vertex<T>& p)->int{
            int cid = 0;
            for (unsigned int d=0; d<D; ++d) {
              double mid = 0.5*(node.bmin[d] + node.bmax[d]).c[0];
              double val; whiteice::math::convert(val, p[d]);
              if (val >= mid) cid |= (1<<d);
            }
            return cid;
          };
          ...
          // insert old point
          int cid_old = child_id_for(y[old_idx]);
          int ch_old = create_child(cid_old);
          nodes[ch_old].point_index = (int)old_idx;
          nodes[ch_old].mass = T(1);
          nodes[ch_old].com = y[old_idx];

          // continue with current insertion in the same loop (node is now internal)
        }

        // insert current point into appropriate child
        auto child_id_for2 = [&](const math::vertex<T>& p)->int{
          int cid = 0;
          for (unsigned int d=0; d<D; ++d) {
            double mid = 0.5*(nodes[node_i].bmin[d] + nodes[node_i].bmax[d]).c[0];
            double val; whiteice::math::convert(val, p[d]);
            if (val >= mid) cid |= (1<<d);
          }
          return cid;
        ...
        int cid = child_id_for2(y[idx]);
        if (nodes[node_i].child[cid] < 0) {
          // create child
          BHNode c;
          c.bmin = nodes[node_i].bmin;
          c.bmax = nodes[node_i].bmax;
          c.child.assign((D==2)?4:8, -1);
          c.com.resize(D); c.com.zero();
          c.mass = T(0);
          c.point_index = -1;
          // bounds for cid
          ...
          int new_idx = (int)nodes.size();
          nodes.push_back(c);
          nodes[node_i].child[cid] = new_idx;
        }
        node_i = nodes[node_i].child[cid];
        // loop continues until placed
      }
    };

    for (unsigned int i = 0; i < N; ++i) insert_point(i);

    // post-order accumulate COM and mass
    std::function<void(int)> accumulate = [&](int ni){
      BHNode& nd = nodes[ni];
      const bool internal = std::any_of(nd.child.begin(), nd.child.end(), [](int c){return c>=0;});
      if (!internal) {
        // leaf: mass/com already set (either empty or single point)
        return;
      }
      nd.mass = T(0);
      nd.com.resize(D); nd.com.zero();
      for (int cidx : nd.child) if (cidx >= 0) {
        accumulate(cidx);
        nd.mass += nodes[cidx].mass;
        nd.com += nodes[cidx].mass * nodes[cidx].com;
      }
      if (nd.mass > T(0)) nd.com /= nd.mass;
    };
    accumulate(root_idx);
    return root_idx;
  }
  ...
  template <typename T>
  void TSNE_BH<T>::bh_repulsive_forces(const std::vector< math::vertex<T> >& y,
                                    const std::vector<BHNode>& nodes,
                                    const int node_idx,
                                    const unsigned int i,
                                    const T theta,
                                    math::vertex<T>& force_i,
                                    T& qsum_i) const
  {
    if (node_idx < 0) return;
    const unsigned int D = y[0].size();
    const BHNode& node = nodes[node_idx];
    if (node.mass == T(0)) return;

    // if leaf with single point and it's i itself -> skip
    if (node.point_index == (int)i && std::all_of(node.child.begin(), node.child.end(), [](int c){return c<0;})) {
      return;
    }

    // distance from i to node COM
    math::vertex<T> delta = y[i] - node.com;
    T dist2 = (delta*delta)[0];
    if (dist2 == T(0)) {
      // jitter to avoid zero division
      const T eps = T(1e-9);
      for (unsigned int d=0; d<D; ++d) delta[d] += ((i%2)?eps:-eps);
      dist2 = (delta*delta)[0];
    }

    // opening condition
    T s; whiteice::math::convert(s, node.size);
    if (s / whiteice::math::sqrt(whiteice::math::real(dist2)) < theta || std::all_of(node.child.begin(), node.child.end(), [](int c){return c<0;})) {
      // approximate this node
      T q = T(1) / (T(1) + dist2);           // unnormalized Student-t kernel
      T mult = node.mass * q * q;            // (1/(1+r^2))^2 * mass
      force_i += (T(4) * mult) * delta;      // raw repulsive (unnormalized, Z will divide later)
      qsum_i += node.mass * q;               // accumulate unnormalized q-sum
    } else {
      // traverse children
      for (int cidx : node.child) if (cidx >= 0) {
        bh_repulsive_forces(y, nodes, cidx, i, theta, force_i, qsum_i);
      }
    }
  }

  template <typename T>
  void TSNE_BH<T>::sparsify_pij(std::vector< std::vector<T> >& pij,
                             unsigned int topK) const
  {
    const unsigned int N = pij.size();
    if (N == 0) return;
    if (topK == 0) return;

    // collect topK per row (excluding diagonal)
    std::vector<std::vector<char>> keep(N, std::vector<char>(N, 0));

    #pragma omp parallel for schedule(auto)
    for (unsigned int i=0;i<N;++i){
      std::vector<std::pair<T,unsigned int>> entries;
      entries.reserve(N-1);
      for (unsigned int j=0;j<N;++j){
        if (i==j) continue;
        if (pij[i][j] > T(0)) entries.emplace_back(pij[i][j], j);
      }
      if (entries.size() > topK){
        std::nth_element(entries.begin(), entries.begin()+topK, entries.end(),
                         [](const auto& a, const auto& b){return a.first > b.first;});
        entries.resize(topK);
      }
      for (const auto& e : entries) keep[i][e.second] = 1;
    }

    // enforce symmetry by union
    #pragma omp parallel for schedule(auto)
    for (unsigned int i=0;i<N;++i){
      for (unsigned int j=0;j<N;++j){
        if (keep[i][j] || keep[j][i]) { keep[i][j]=1; keep[j][i]=1; }
      }
    }

    // zero others; accumulate sum for renormalization
    double tsum = 0.0;
    #pragma omp parallel for schedule(auto) reduction(+:tsum)
    for (unsigned int i=0;i<N;++i){
      for (unsigned int j=0;j<N;++j){
        if (i==j) { pij[i][j] = T(0); continue; }
        if (!keep[i][j]) pij[i][j] = T(0);
        tsum += pij[i][j].c[0];
      }
    }
    if (T(tsum) > T(0)){
      const T inv = T(1)/T(tsum);
      #pragma omp parallel for schedule(auto)
      for (unsigned int i=0;i<N;++i)
        for (unsigned int j=0;j<N;++j)
          pij[i][j] *= inv;
  ...
  template <typename T>
  bool TSNE_BH<T>::bh_kl_gradient(const std::vector< std::vector<T> >& pij,
                               const std::vector< math::vertex<T> >& y,
                               std::vector< math::vertex<T> >& ygrad,
                               T& Z_out,
                               const T theta) const
  {
    ...
    if (N == 0) return false;
    const unsigned int D = y[0].size();
    if (D < 2 || D > 3) return false; // supports 2D/3D

    // Build tree
    std::vector<BHNode> nodes;
    int root = bh_build_tree(y, nodes);
    if (root < 0) return false;

    // Repulsive forces and q-sum (unnormalized) per point
    std::vector< math::vertex<T> > frep_raw(N);
    std::vector<T> qsum_i(N, T(0));

    #pragma omp parallel for schedule(auto)
    for (unsigned int i=0; i<N; ++i){
      frep_raw[i].resize(D); frep_raw[i].zero();
      T qsi = T(0);
      bh_repulsive_forces(y, nodes, root, i, theta, frep_raw[i], qsi);
      qsum_i[i] = qsi;
    }

    // Normalization constant Z = sum_{i,j} q_unnorm(i,j)
    T Z = T(0);
    for (unsigned int i=0;i<N;++i) Z += qsum_i[i];
    // NOTE: this double-counts pairs (i,j) and (j,i), consistent with q_ij in gradient formula
    if (Z == T(0)) Z = T(1);
    Z_out = Z;

    // Attractive forces using P (sparse-friendly). If P is dense, this is O(N^2).
    std::vector< math::vertex<T> > fattr(N);
    #pragma omp parallel for schedule(auto)
    for (unsigned int i=0;i<N;++i){
      fattr[i].resize(D); fattr[i].zero();
      for (unsigned int j=0;j<N;++j){
        if (i==j) continue;
        if (pij[i][j] == T(0)) continue; // skips pruned entries
        math::vertex<T> delta = y[i] - y[j];
        T dist2 = (delta*delta)[0];
        T q = T(1) / (T(1) + dist2);
        fattr[i] += (T(4) * pij[i][j] * q) * delta;
      }
    }

    // Final gradient = attractive - repulsive (repulsive normalized by Z)
    ygrad.resize(N);
    #pragma omp parallel for schedule(auto)
    for (unsigned int i=0;i<N;++i){
      ygrad[i].resize(D);
      ygrad[i] = fattr[i] - (frep_raw[i] / Z);
    }
    return true;
  }
  ...
  template class TSNE_BH< math::blas_real<float> >;
  template class TSNE_BH< math::blas_real<double> >;
